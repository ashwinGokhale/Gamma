...
const initProgram = () => {
	...
	program
		...
		.alias("a")
		.description(
			"Adds the specified bases as well as all git repos in them"
		)
		.action(add);
	...
	program
		...
		.option(
			"-b, --base <base>",
			"Sets the base of the context. Can fuzzy match base"
		)
		.option(
			"-r, --repo <repo>",
		...
		.description(
			"Set the base and/or repo of the context. Supports fuzzy matching"
		)
		.action(set);
	...
	program
		...
		.alias("re")
		.description(
			"Rescan base(s) and removes any base or repo that does not exist anymore"
		)
		.action(rebase);
	...
	program
		...
		.alias("d")
		.description(
			"Runs background process that maintains all bases and their repos"
		)
		.action(daemon);
