...
export const add = (base = "", bases = []) => {
	...
	bases = filter(
		bases.map(b => path.resolve(et(b))).sort((a, b) => a.length - b.length)
	);
	let to_index = [];
	bases.forEach(base => {
...
export const remove = (base = "", bases = []) => {
	...
	bases.concat(base).forEach(base => {
		...
		} else console.log(chalk.red(`${base_path} is not a base`));
	});
...
export const search = (base = "") => {
	...
	Object.keys(dotfile["bases"]).forEach(b =>
		Object.entries(dotfile["bases"][b]["repos"]).forEach(key =>
			repos.push(key[1]["path"])
		)
	);
...
`;
	...
	for (let shellPath of ["~/.bashrc", "~/.zshrc"]) {
		...
		try {
			...
			messages.push(
				chalk.green(
					`${name} command has been installed in ${shellPath}`
				)
			);
		} catch (error) {
...
export const rebase = (bases = []) => {
	...
	bases = bases.length
		? bases
				.map(b => path.resolve(et(b)))
				.filter(base => dotfile["bases"][base])
		: dotBases;
	...
	if (
		...
		!fs.existsSync(dotfile["context"]["base"])
	)
	...
	if (
		...
		!fs.existsSync(dotfile["context"]["repo"]["path"])
	)
		dotfile["context"]["repo"] = {};
...
export const daemon = () => {
	ps.lookup(
		{ command: "node", arguments: ["gamma", "daemon"] },
		(err, processes) => {
			...
			chokidar
				...
				.on("unlinkDir", dirPath => {
					...
					for (let base of Object.keys(dotfile["bases"])) {
						...
						delete dotfile["bases"][base]["repos"][
							path.basename(dirPath)
						];
						dumpDotfile(dotfile);
						break;
...
export const status = () => {
	...
	Object.keys(dotfile["bases"]).forEach(base => {
		...
		Object.keys(dotfile["bases"][base]["repos"]).forEach(repoName => {
			...
			console.log(
				`${repoName}${" ".repeat(
					Math.abs(pad - repoName.length + 11)
				)}${messages.join(" | ")}`
			);
		});
		console.log();
